    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div style={{ background: 'white', borderRadius: 16, padding: 24, width: 400 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <p style={{ fontSize: 12, color: '#9ca3af', margin: 0, background: '#f9fafb', padding: '8px 12px', borderRadius: 8 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {amount && members.length > 0 && (
            <div style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 12px' }}>
              <p style={{ fontSize: 12, color: '#6b7280', margin: '0 0 6px' }}>Divisão igualitária entre {memberCount} pessoa{memberCount !== 1 ? 's' : ''}:</p>
              {members.map((m, i) => (
                <div key={m.user_id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '2px 0' }}>
                  <span style={{ color: '#374151' }}>{m.profiles?.display_name || 'Membro'}</span>
    ...
    <div style={{ background: 'white', borderRadius: 12, border: '0.5px solid #e5e7eb', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '1rem 1.25rem', cursor: 'pointer' }} onClick={() => setExpanded(!expanded)}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={{ fontSize: 14, fontWeight: 500, margin: 0, color: '#111827' }}>{group.name}</p>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 500, color: '#374151' }}>{fmt(totalSpent)}</span>
          ...
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {transfers.map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                <span style={{ padding: '2px 8px', background: '#fef2f2', color: '#ef4444', borderRadius: 4, fontWeight: 500 }}>{t.from}</span>
                <span style={{ color: '#9ca3af' }}>deve</span>
                <span style={{ padding: '2px 8px', background: '#f0fdf4', color: '#10b981', borderRadius: 4, fontWeight: 500 }}>{fmt(t.amount)}</span>
                <span style={{ color: '#9ca3af' }}>a</span>
                <span style={{ color: '#374151', fontWeight: 500 }}>{t.to}</span>
              </div>
            ))}
          </div>
        )}
        {transfers.length === 0 && expenses.length > 0 && (
          <p style={{ fontSize: 12, color: '#10b981', marginTop: 8, margin: '8px 0 0' }}>✅ Tudo liquidado!</p>
        )}
      </div>
      ...
      {expanded && (
        <div style={{ borderTop: '0.5px solid #f3f4f6' }}>
          {/* Ações */}
          <div style={{ padding: '10px 1.25rem', display: 'flex', gap: 8, borderBottom: '0.5px solid #f3f4f6' }}>
          ...
          <div style={{ padding: '0 1.25rem' }}>
            {expenses.length === 0 ? (
              <p style={{ fontSize: 13, color: '#9ca3af', padding: '1rem 0' }}>Sem despesas ainda.</p>
            ) : (
              expenses.map(exp => {
                const settled = exp.shared_expense_splits?.every(s => s.is_settled)
                return (
                  <div key={exp.id} style={{ padding: '10px 0', borderBottom: '0.5px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 500, margin: 0, color: settled ? '#9ca3af' : '#111827', textDecoration: settled ? 'line-through' : 'none' }}>
                        {exp.description}
                      </p>
                      <p style={{ fontSize: 11, color: '#9ca3af', margin: 0 }}>
                        Pago por {exp.profiles?.display_name || 'alguém'} · {new Date(exp.date).toLocaleDateString('pt-PT')}
                      </p>
                      {exp.shared_expense_splits?.map(s => (
                        <span key={s.user_id} style={{ fontSize: 10, marginRight: 6, color: s.is_settled ? '#10b981' : '#9ca3af' }}>
                          {s.profiles?.display_name}: {fmt(s.amount)} {s.is_settled ? '✓' : ''}
                        </span>
                      ))}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      <span style={{ fontSize: 13, fontWeight: 500, color: '#374151' }}>{fmt(exp.amount)}</span>
                      {!settled && (
                        <button onClick={() => settle.mutate(exp.id)}
                          style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #d1fae5', background: '#f0fdf4', color: '#10b981', fontSize: 11, cursor: 'pointer' }}>
                          Liquidar
                        </button>
                      )}
                    </div>
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0,1fr))', gap: 12, marginBottom: 24 }}>
        <div style={{ background: 'white', borderRadius: 12, border: '0.5px solid #e5e7eb', padding: '1rem 1.25rem' }}>
          <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 4px' }}>Total em grupos</p>
          <p style={{ fontSize: 24, fontWeight: 500, color: '#6366f1', margin: 0 }}>{fmt(totalOwed)}</p>
        </div>
        <div style={{ background: 'white', borderRadius: 12, border: '0.5px solid #e5e7eb', padding: '1rem 1.25rem' }}>
          <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 4px' }}>Grupos ativos</p>
          <p style={{ fontSize: 24, fontWeight: 500, color: '#374151', margin: 0 }}>{groups.filter(g => !g.settled_at).length}</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: 'center', padding: '3rem 0' }}>
          ...
          <p style={{ color: '#9ca3af', fontSize: 14, marginBottom: 12 }}>Sem grupos ainda. Cria um para dividir despesas!</p>
          <button onClick={() => setShowGroupForm(true)}
